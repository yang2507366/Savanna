...
function UIGridView:create(numberOfColumns--[[option]])
    self = self:get(runtime::invokeClassMethod("LIGridView", "create:", AppContext.current()));
    if numberOfColumns == nil then
        numberOfColumns = 3;
    end
    self:setNumberOfColumns(numberOfColumns);
    
    UIGridViewEventProxyTable[self:id()] = self;
