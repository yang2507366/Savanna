...
- (NSString *)userDataKeyForTask:(id<SVTaskExecutable>)task;

@end

@implementation SVTaskQueue
...
{
    return [[[SVTaskQueue alloc] init] autorelease];
}
...
- (void)executeNextTask
{
    if([self.taskList count] != 0){
        id<SVTaskExecutable> task = [[self.taskList objectAtIndex:0] retain];
...
- (void)skipTask:(NSInteger)count
{
    for(NSInteger i = 0; i < count && self.taskList.count != 0; ++i){
        id<SVTaskExecutable> task = [self.taskList objectAtIndex:0];
        [self removeTask:task];
    }
}
