...
@interface SVToast () <SVDelayControllerDelegate>
...
@property(nonatomic, retain)SVDelayController *animationDelayController;
...
- (void)showToastInView:(UIView *)parentView withString:(NSString *)string hideAfterInterval:(NSTimeInterval)interval
{
    ...
    self.delayController = [[[SVDelayController alloc] initWithInterval:interval] autorelease];
...
- (void)delayControllerDidFinishDelay:(SVDelayController *)controller
{
    if(controller == self.delayController){
        ...
        self.animationDelayController = [[[SVDelayController alloc] initWithInterval:animationDuration] autorelease];
