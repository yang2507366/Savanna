...
+ (NSString *)identifierForObj:(id)obj
{
    ...
    return [NSString stringWithFormat:@"id_%d", arc4random() % 1000];
}
...
+ (void)addProviderToSharedPool:(id<SVProviderPoolable>)provider
{
    [self addProviderToSharedPool:provider identifier:nil];
}
