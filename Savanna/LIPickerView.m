...
- (void)setNumberOfComponentsFunc:(NSString *)func
{
    if(func.length != 0){
        [self setNumOfComponents:^NSInteger{
            NSString *num = [[SVAppManager scriptInteractionWithAppId:self.appId] callFunction:func parameters:self.objId, nil];
            return [num intValue];
        }];
    }else{
        self.numOfComponents = nil;
...
- (void)setNumberOfRowsInComponentFunc:(NSString *)func
{
    if(func.length != 0){
        [self setNumOfRowsInComponent:^NSInteger(NSInteger component) {
            NSString *rows = [[SVAppManager scriptInteractionWithAppId:self.appId]
...
- (void)setWidthForComponentFuc:(NSString *)func
{
    if(func.length != 0){
        [self setWidthForComponent:^CGFloat(NSInteger component) {
            NSString *width = [[SVAppManager scriptInteractionWithAppId:self.appId]
...
- (void)setRowHeightForComponentFunc:(NSString *)func
{
    if(func.length != 0){
        [self setRowHeightForComponent:^CGFloat(NSInteger component) {
            NSString *rowHeight = [[SVAppManager scriptInteractionWithAppId:appId]
                                   callFunction:func parameters:objId, [NSString stringWithFormat:@"%d", component], nil];
...
- (void)setTitleForRowForComponentFunc:(NSString *)func
{
    if(func.length != 0){
        [self setTitleForRowForComponent:^NSString *(NSInteger row, NSInteger componnet) {
            NSString *title = [[SVAppManager scriptInteractionWithAppId:appId]
...
- (void)setViewForRowForComponentReusingViewFunc:(NSString *)func
{
    if(func.length!= 0){
        [self setViewForRowForComponentReuseView:^UIView *(NSInteger row, NSInteger component, UIView *reusingView) {
            ...
            if(reusingView){
                reusingViewId = [SVObjectManager addObject:reusingView group:appId];
            }
            NSString *viewId = [[SVAppManager scriptInteractionWithAppId:appId]
            ...
            if(reusingView){
                [SVObjectManager releaseObjectWithId:reusingViewId group:appId];
            }
            UIView *view = [SVObjectManager objectWithId:viewId group:appId];
            return view;
        }];
    }else{
        self.viewForRowForComponentReuseView = nil;
...
+ (NSString *)create:(NSString *)appId
{
    ...
    impl.appId = appId;
    impl.objId = [SVObjectManager addObject:impl group:appId];
