...
@interface SVLuaScriptInteraction () {
    char *script_string;
    lua_State *lua_state;
}
...
@implementation SVLuaScriptInteraction
...
int get_module(lua_State *L)
{
    ...
    if(strlen(cModName) != 0){
        ...
        NSString *appId = @"";
        NSInteger beginIndex = [requireString sv_find:lua_require_separator];
        ...
        if(beginIndex != -1){
            ...
            targetScript = [SVAppManager scriptWithScriptName:modName appId:appId];
        }
...
{
    self = [self initWithScript:script errorMsgThrowBlock:nil];
    return self;
}
...
- (id)initWithScript:(NSString *)script errorMsgThrowBlock:(void(^)(NSString *))errorMsgThrowBlock
{
    ...
    self.script = script;
    self.errorMsgThrowBlock = errorMsgThrowBlock;
    self.scriptInvokeFilter = [[SVUnicodeScriptInvokeFilter new] autorelease];
...
- (void)initializeLuaState
{
    ...
    if(script_string){
        if(luaL_dostring(lua_state, script_string)){
            // dostring error, lua cannot do this script
            const char *errorMsg = lua_tostring(lua_state, -1);
            if(self.errorMsgThrowBlock){
                NSString *errorString = [NSString stringWithFormat:@"%s", errorMsg];
                self.errorMsgThrowBlock(errorString);
            }
        }
    }
}
