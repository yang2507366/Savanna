...
@implementation SVLuaRuntimeUtils
...
{
    id object = [SVObjectManager objectWithId:objectId group:group];
#ifdef DEBUGLOG
//    DEBUGLOG(@"%@ %@ \t%@", object, methodName, parameters);
#endif
    ...
    if(methodSignature){
        ...
        for(NSInteger i = 2, j = 0; i < numberOfArguments && j < parameters.count; ++i, ++j){
            ...
            if(ctype == 'c' || ctype == 'B'){//char
                ...
                id obj = tmpParam;//****************************
                if([SVLuaCommonUtils isObjCObject:tmpParam]){
                    obj = [SVObjectManager objectWithId:tmpParam group:group];
                }
                if(tmpParam.length == 0){
                    obj = nil;
                }
...
#ifdef DEBUGLOG
//                DEBUGLOG(@"set struct:%s", [methodSignature getArgumentTypeAtIndex:i]);
#endif
                NSString *structType = [NSString stringWithFormat:@"%s", [methodSignature getArgumentTypeAtIndex:i]];
                if([structType hasPrefix:@"{CGSize"]){
                    CGSize size = [SVLuaCommonUtils CGSizeWithString:tmpParam];
                    argumentData = &size;
                }else if([structType hasPrefix:@"{CGPoint"]){
                    CGPoint point = [SVLuaCommonUtils CGPointWithString:tmpParam];
                    argumentData = &point;
                }else if([structType hasPrefix:@"{CGRect"]){
                    CGRect rect = [SVLuaCommonUtils CGRectWithString:tmpParam];
                    argumentData = &rect;
                }else if([structType hasPrefix:@"{NSRange"]){
                    NSRange range = [SVLuaCommonUtils NSRangeWithString:tmpParam];
                    argumentData = &range;
                }else if([structType hasPrefix:@"{UIEdgeInsets"]){
                    UIEdgeInsets insets = [SVLuaCommonUtils UIEdgeInsetsWithString:tmpParam];
                    argumentData = &insets;
                }else if([structType hasPrefix:@"{UIOffset"]){
                    UIOffset offset = [SVLuaCommonUtils UIOffsetWithString:tmpParam];
                    argumentData = &offset;
                }else if([structType hasPrefix:@"{CATransform3D"]){
                    CATransform3D t3d = [SVLuaCommonUtils CATransform3DWithString:tmpParam];
                    argumentData = &t3d;
                }else if([structType hasPrefix:@"{CGAffineTransform"]){
                    CGAffineTransform t = [SVLuaCommonUtils CGAffineTransformWithString:tmpParam];
                    argumentData = &t;
                }
            }else if(ctype == '('){//union
            }else if(ctype == 'b'){//bit
            ...
            if(ctype == 'c'){//char
                ...
                if(obj){
                    if([obj isKindOfClass:NSString.class]){
                        ...
                        NSString *objId = [SVObjectManager addObject:obj group:group];
                        return objId;
                    }
                }
                return obj;//*************************************
...
#ifdef DEBUGLOG
//                DEBUGLOG(@"get struct:%s", [methodSignature methodReturnType]);//未处理
#endif
                         ...
                         parameters:(NSArray *)parameters
{
    ...
    if(object){
        NSString *objId = [SVObjectManager addObject:object group:group];
        ...
        if(resultObjId.length == 0){
            [SVObjectManager removeObjectWithId:objId group:group];
        }else{
            [SVObjectManager releaseObjectWithId:resultObjId group:group];
        }
        [object autorelease];
        return resultObjId;
    }
