...
- (void)tabBarController:(UITabBarController *)tabBarController didSelectViewController:(UIViewController *)viewController
{
    if(self.didSelectViewController.length != 0){
        [[SVAppManager scriptInteractionWithAppId:self.appId] callFunction:self.didSelectViewController parameters:self.objId, nil];
    }
}
...
- (BOOL)tabBarController:(UITabBarController *)tabBarController shouldSelectViewController:(UIViewController *)viewController
{
    if(self.shouldSelectViewController.length != 0){
        return [[[SVAppManager scriptInteractionWithAppId:self.appId] callFunction:self.shouldSelectViewController parameters:self.objId, nil] boolValue];
    }
    return YES;
}
