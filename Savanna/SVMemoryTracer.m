...
#import "SVMemoryTracer.h"
#import "SVSystemUtils.h"

@interface SVMemoryTracer ()
...
- (void)mark
{
    self.bytesMemoryOfStartup = [SVSystemUtils bytesOfMemoryUsed];
}
...
- (void)trace
{
    ...
    self.displayLabel.text = [NSString stringWithFormat:@"%uk", ([SVSystemUtils bytesOfMemoryUsed] - self.bytesMemoryOfStartup) / 1024];
    if(self.tracing){
...
+ (void)mark
{
    SVMemoryTracer *instance = [SVMemoryTracer sharedInstance];
    [instance mark];
}
