...
{
    NSString *func = @"function ";
    NSInteger beginIndex = [script sv_find:func fromIndex:index];
    if(beginIndex != -1){
        NSInteger endIndex = [script sv_find:@"(" fromIndex:index];
        if(endIndex != -1){
            NSString *funcName = [script sv_substringWithBeginIndex:beginIndex + func.length endIndex:endIndex];
...
{
    NSInteger beginIndex = [func sv_find:@"("];
    NSInteger endIndex = [func sv_find:@")"];
    if(beginIndex != -1 && endIndex != -1){
        return [func sv_substringWithBeginIndex:beginIndex + 1 endIndex:endIndex];
    }
    return @"";
}
...
{
    NSString *func = @"function ";
    NSInteger functionBeignIndex = [script sv_find:func fromIndex:index reverse:YES];
    if(functionBeignIndex != -1){
        functionBeignIndex += func.length;
        NSInteger endIndex = [script sv_find:@"(" fromIndex:functionBeignIndex];
        NSString *funcName = [script sv_substringWithBeginIndex:functionBeignIndex endIndex:endIndex];
        NSArray *arr = [funcName componentsSeparatedByString:@":"];
        if(arr.count == 2){
...
- (NSString *)checkScript:(NSString *)script scriptName:(NSString *)scriptName bundleId:(NSString *)bundleId
{
    ...
    while((beginIndex = [script sv_find:@"super" fromIndex:lastFindIndex]) != -1){
        ...
        if([SVLuaCommonUtils isAlphbelt:leftChar] || [SVLuaCommonUtils isAlphbelt:rightChar]){
            [resultString appendString:[script sv_substringWithBeginIndex:lastFindIndex endIndex:beginIndex + 5]];
            lastFindIndex = beginIndex + 5;
        }else if(parentClassName.length != 0){
            // right value
            BOOL selfSyntax = rightChar == ':';
            [resultString appendString:[script sv_substringWithBeginIndex:lastFindIndex endIndex:beginIndex]];
            [resultString appendFormat:@"getmetatable(%@)", parentClassName];
            if(selfSyntax){
                [resultString appendString:@"."];
                NSInteger endIndex = [script sv_find:@"(" fromIndex:beginIndex];
                if(endIndex == -1){
                    // error syntax
                    return script;
                }
                NSString *invokeFuncName = [script sv_substringWithBeginIndex:beginIndex + 6 endIndex:endIndex];
                [resultString appendString:invokeFuncName];
                NSInteger rightBracketIndex = [script sv_find:@")" fromIndex:endIndex];
                [resultString appendString:@"(self"];
                NSString *innerParams = [self getFuncInnerParams:[script sv_substringWithBeginIndex:beginIndex endIndex:rightBracketIndex + 1]];
    ...
    if(lastFindIndex != -1 && lastFindIndex < script.length){
        [resultString appendString:[script sv_substringWithBeginIndex:lastFindIndex endIndex:script.length]];
    }
    return resultString;
}
