...
- (BOOL)popoverControllerShouldDismissPopover:(UIPopoverController *)popoverController
{
    if(self.shouldDismiss.length != 0){
        [[SVAppManager scriptInteractionWithAppId:self.appId] callFunction:self.shouldDismiss parameters:self.objId, nil];
    }
    return YES;
}
...
- (void)popoverControllerDidDismissPopover:(UIPopoverController *)popoverController
{
    if(self.didDismiss.length != 0){
        [[SVAppManager scriptInteractionWithAppId:self.appId] callFunction:self.didDismiss parameters:self.objId, nil];
    }
}
