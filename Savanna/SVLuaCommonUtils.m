...
@implementation SVLuaCommonUtils

+ (BOOL)charIndexInCommentBlockWithScript:(NSString *)script index:(NSInteger)index
{
    BOOL noComment = YES;
    NSInteger commentBeginIndex = [script sv_find:@"--[[" fromIndex:index reverse:YES];
    if(commentBeginIndex != -1){
        // find --[[ comment
        NSInteger commentEndIndex = [script sv_find:@"]]" fromIndex:index];
    ...
    if(noComment){
        // find -- comment
        NSInteger newLineIndex = [script sv_find:@"\n" fromIndex:index reverse:YES];
        if(newLineIndex == -1){
            newLineIndex = 0;
        }else{
            ++newLineIndex;
        }
        NSString *innerStr = [script sv_substringWithBeginIndex:newLineIndex endIndex:index];
        if([innerStr sv_find:@"--"] != -1){
            // finded
            noComment = NO;
        }
    }
...
{
    NSInteger beginIndex = [script sv_find:@"function"];
    NSInteger endIndex = 0;
    while(beginIndex != -1){
        endIndex = [script sv_find:@"(" fromIndex:beginIndex];
        if(endIndex != -1){
            NSString *funcName = [script sv_substringWithBeginIndex:beginIndex + 8 endIndex:endIndex];
            ...
            beginIndex = [script sv_find:@"function" fromIndex:endIndex + 2];
        }else{
            break;
        }
    }
...
{
    return [SVCommonUtils isAlphbelt:c];
}

+ (BOOL)isAlphbelts:(NSString *)str
{
    return [SVCommonUtils isAlphbelts:str];
}
