...
@interface SVDatabaseKeyValueManager ()
...
@implementation SVDatabaseKeyValueManager
...
- (id)init
{
    NSString *randomCacheName = [SVCommonUtils randomString];
...
{
    NSString *filePath = [[SVCommonUtils libraryPath] 
                          stringByAppendingPathComponent:[SVEncryptUtils hexStringByMD5EncryptingString:dbName]];
...
{
    dbName = [SVEncryptUtils hexStringByMD5EncryptingString:dbName];
...
- (void)openDatabase:(NSString *)dbFilePath
{
    ...
    if(sqlite3_open([dbFilePath UTF8String], &_db) == SQLITE_OK){
        if(!dbExists){
#ifdef DEBUGLOG
            DEBUGLOG(@"create cache:%@", self.dbName);
#endif
...
#ifdef DEBUGLOG
                DEBUGLOG(@"create table for cache:%@ succeed", self.dbName);
#endif
            }else{
#ifdef DEBUGLOG
                DEBUGLOG(@"create table for cache:%@ failed", self.dbName);
#endif
            }
        }
    }
...
- (void)setValue:(NSString *)value forKey:(NSString *)key
{
    ...
    value = [SVEncryptUtils hexStringByEncodingString:value];
    key = [SVEncryptUtils hexStringByEncodingString:key];
...
//        NSLog(@"%@", sql);
    ...
    if(errMsg){
#ifdef DEBUGLOG
        DEBUGLOG(@"%@", [NSString stringWithUTF8String:errMsg]);
#endif
    }
}
...
//            NSLog(@"%@", sql);
    ...
    if(value){
        value = [SVEncryptUtils stringByDecodingHexString:value];
    }
...
- (NSArray *)selectKeyBySQL:(NSString *)sql
{
    ...
    if(sqlite3_prepare(_db, [sql UTF8String], -1, &stmt, NULL) == SQLITE_OK){
        ...
        while(sqlite3_step(stmt) == SQLITE_ROW){
            ...
            key = [SVEncryptUtils stringByDecodingHexString:key];
