...
@implementation SVGoogleGeocoder
...
//             NSLog(@"%@", responseString);
        ...
        if(address.length != 0 || locality.length != 0){
            [self notifySuccessWithLocality:locality address:address];
        }else{
...
- (void)notifySuccessWithLocality:(NSString *)locality address:(NSString *)address
{
    if([self.delegate respondsToSelector:@selector(geocoder:didRecieveLocality:)]){
        SVLocationDescription *desc = [[[SVLocationDescription alloc] init] autorelease];
